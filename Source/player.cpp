...
void StartAttack(Player &player, Direction d, bool includesFirstFrame)
{
	...
	if (includesFirstFrame) {
		if (HasAnyOf(player._pIFlags, ItemSpecialEffect::FastestAttack) && HasAnyOf(player._pIFlags, ItemSpecialEffect::QuickAttack | ItemSpecialEffect::FastAttack)
		    ...
		    || player.InvBody[INVLOC_HAND_LEFT]._itype == ItemType::Staff && player.InvBody[INVLOC_HAND_LEFT]._iMagical == ITEM_QUALITY_UNIQUE && player._pClass == HeroClass::Monk
		        && player._pLevel >= 30 && HasAnyOf(player._pIFlags, ItemSpecialEffect::Empower)) {
		...
		if (HasAnyOf(player._pIFlags, ItemSpecialEffect::FasterAttack)
		    ...
		    || HasInventoryItemWithId(player, IDI_AURIC) && HasAnyOf(player._pIFlags, ItemSpecialEffect::Empower)
		    || HasAnyOf(player._pIFlags, ItemSpecialEffect::QuickAttack)) {
			skippedAnimationFrames = 1;
		} else if (HasAnyOf(player._pIFlags, ItemSpecialEffect::FastestAttack)
		    || player.InvBody[INVLOC_HAND_LEFT]._itype == ItemType::Staff && player.InvBody[INVLOC_HAND_LEFT]._iMagical == ITEM_QUALITY_UNIQUE && player._pClass == HeroClass::Monk
		        && player._pLevel >= 30 && HasAnyOf(player._pIFlags, ItemSpecialEffect::Empower)) {
...
bool DoAttack(Player &player)
{
	...
	if (player.AnimInfo.currentFrame == player._pAFNum - 1) {
		...
		if (&player == MyPlayer && PlayerAtPosition(position) != nullptr) {
			...
			if ((GenerateRnd(100) + 1) <= HolyFireChancePlr(player, *PlayerAtPosition(position))) {
				HolyFireDamagePlr(player, *PlayerAtPosition(position));
				CastHolyShockPlr(player, *PlayerAtPosition(position));
				ExplodingBoneArmorPlr(player, *PlayerAtPosition(position));
			}
...
bool DoRangeAttack(Player &player)
{
	...
	if (player.AnimInfo.currentFrame == player._pAFNum - 1) {
		arrows = HasAnyOf(player._pIFlags, ItemSpecialEffect::MultipleArrows)
		        || HasAnyOf(player._pIFlags, ItemSpecialEffect::Empower) && player._pIMisType > 0
		    ? 3
		    : 1;
	}
...
StartPlayerKill(Player &player, DeathReason deathReason)
{
	...
	const bool dropItems = dropGold && deathReason == DeathReason::Player && *sgOptions.Gameplay.friendlyFire == true
	    || dropGold && deathReason == DeathReason::MonsterOrTrap && *sgOptions.Gameplay.friendlyFire == true;
...
void ApplyPlrDamage(DamageType damageType, Player &player, int dam, int minHP /*= 0*/, int frac /*= 0*/, DeathReason deathReason /*= DeathReason::MonsterOrTrap*/)
{
	...
	if (totalDamage > 0 && player.pManaShield) {
		...
		int damageToHealth = damageAfterReduction - damageRedirectedToMana;
		...
		if (player._pMana >= damageRedirectedToMana) {
			...
			damageToHealth += damageRedirectedToMana - player._pMana;
			player._pMana = 0;
